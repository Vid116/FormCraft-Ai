...
//  Main Component
// ═══════════════════════════════════════════
export function FormResponsesView({ form, responses: initialResponses, plan = "free" }: { form: Form; responses: FormResponse[]; plan?: PlanId }) {
  const canCSV = hasFeatureAccess(plan, "csv_export");
  ...
  async function generateSummary() {
    ...
    try {
      ...
      if (res.status === 429) {
        setSummary(`You've used ${data.quota?.current ?? "all"} of ${data.quota?.limit ?? "your"} AI summaries this month. Upgrade your plan for more.`);
      } else {
      ...
      <div className="flex items-center gap-6 border-b border-zinc-200 dark:border-zinc-800">
        ...
        <div className="ml-auto flex items-center gap-3 pb-2.5">
          ...
          {canCSV ? (
            ...
            <span className="text-xs text-zinc-400 flex items-center gap-1 cursor-not-allowed" title="Upgrade to Business for CSV export">
